---
layout: post
title: "Folland Week 33 — Sobolev Spaces — Fourier and Hilbert Structure"
permalink: /study/folland/week-33-sobolev-spaces-fourier-and-hilbert-structure/
math: true
toc: true
status: outline
date: 2026-09-11 00:00:00 +0900
categories: [Mathematics]
tags: [analysis, folland]
note_type: seminar
series: folland-seminar
series_order: 33
---

- **Main topic:** Sobolev Spaces — Fourier and Hilbert Structure
- **Reference:** Folland 2nd edition, §9.3 Part I
- **Prerequisite checklist:** [미확인 선행 내용]({% link study/folland/prerequisite-checklist.md %})

## Prerequisite

- [Week 28 — Distributions — Test Functions and Derivatives]({% link _posts/folland-seminar/2026-09-11-folland-week-28-distributions-test-functions-and-derivatives.md %})
- [Week 31 — Tempered Distributions and Fourier Transform]({% link _posts/folland-seminar/2026-09-11-folland-week-31-tempered-distributions-and-fourier-transform.md %})
- [Week 20 — Fourier Inversion and Plancherel]({% link _posts/folland-seminar/2026-09-11-folland-week-20-fourier-inversion-and-plancherel.md %})
- [Week 05 — Duality of Lp]({% link _posts/folland-seminar/2026-09-11-folland-week-05-duality-of-lp.md %})
- [Inner Product Geometry]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-05-inner-product-geometry.md %})

## Must cover

- [ ] Folland Hs(Rn) convention
- [ ] distributional derivatives and integer-order norms
- [ ] Fourier multiplier definition; equivalent norms
- [ ] Hilbert structure; density
- [ ] Sobolev duality; negative orders

## Seminar workload

- 총 165분
- integer-order / Fourier definition: 55분
- Hilbert structure / density: 50분
- duality / negative orders: 60분

## Motivation

## Definitions

## Main Results

## Proofs

## Examples / Counterexamples

## Connections

- Dependency: [Orthogonal Decomposition]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-06-orthogonal-decomposition.md %})

- [ ] Connection: Folland Hs(Rn) convention의 선행 결과와 현재 theorem의 연결
- [ ] Next readiness: Sobolev duality; negative orders → Sobolev Embedding Theorem

## Questions / Things I do not understand yet

## Final reconstruction

## Previous Week / Next Week

- Previous (읽기 순서): [Week 32 — Periodic Distributions and Fourier Coefficients]({% link _posts/folland-seminar/2026-09-11-folland-week-32-periodic-distributions-and-fourier-coefficients.md %})
- Current: Week 33
- Next (읽기 순서): [Week 34 — Sobolev Embedding and Localization]({% link _posts/folland-seminar/2026-09-11-folland-week-34-sobolev-embedding-and-localization.md %})
