...
partial pivoting과 permutation bookkeeping을 구현한다.
...
- PA=LU와 A=PLU 중 구현은 어느 convention을 쓰는가?
- k번째 행 교환에서 이미 계산한 L의 어느 부분을 바꿔야 하는가?

## 핵심 내용 체크리스트

- [ ] partial pivoting의 선택 규칙
- [ ] 순열 적용, factorization, solve의 일관성
...
**예정 대상:** `nla::lu_factor_partial_pivoting` in `systems.*`
...
**확인할 비교:** 작은 첫 pivot을 가진 행렬에서 no-pivot/partial-pivot 결과를 비교한다.

### 지표

- PA−LU의 상대 Frobenius norm
- 작은 pivot 사례의 backward error와 행 교환 결과
...
**P의 방향, L의 이전 열 swap, pivot index 오류**
...
- 이전: [Lecture 20 — Gaussian Elimination]({% link _posts/trefethen/2026-09-15-trefethen-20-gaussian-elimination.md %})
- 다음: [Lecture 22 — Stability of Gaussian Elimination]({% link _posts/trefethen/2026-09-15-trefethen-22-stability-of-gaussian-elimination.md %})
