...
full-column-rank least-squares 문제를 Householder QR로 푼다.
...
- 배경: full-column-rank 직사각 행렬

## 먼저 답할 질문

- 최소제곱 해에서 잔차는 왜 range(A)에 직교하는가?
- QR을 사용하면 원래 문제를 어떤 삼각 문제로 줄일 수 있는가?

## 핵심 내용 체크리스트

- [ ] m≥n, full column rank일 때 해의 존재·유일성
- [ ] normal equations 유도와 QR 풀이의 차이
...
**예정 대상:** `nla::least_squares_qr` in `qr.*`
...
**확인할 비교:** 해 residual이 range(A)에 직교하는지와 Eigen 기준해 오차를 확인한다.

### 지표

- 잔차 2-norm과 A* r의 norm
- 기준 QR·SVD 해와의 차이 및 rank-deficient 입력 처리
...
**normal equations를 기본 풀이로 사용하거나 rank deficiency를 무시**
...
- 이전: [Lecture 10 — Householder Triangularization]({% link _posts/trefethen/2026-09-15-trefethen-10-householder-triangularization.md %})
- 다음: [Lecture 12 — Conditioning and Condition Numbers]({% link _posts/trefethen/2026-09-15-trefethen-12-conditioning-and-condition-numbers.md %})
