...
안정적인 Householder reflector와 reduced QR을 구현한다.
...
- 배경: 벡터 정규화와 rank-one update

## 먼저 답할 질문

- Householder reflector의 부호·위상 선택은 cancellation과 어떤 관계인가?
- Q를 만들지 않고 reflector를 적용하려면 무엇을 저장하는가?

## 핵심 내용 체크리스트

- [ ] reflector 구성과 열 소거 과정
- [ ] implicit Q 적용, 연산량, 추가 저장량
...
**예정 대상:** `nla::householder_qr` in `qr.*`
...
**확인할 비교:** reflector sign 선택에 따른 cancellation, reconstruction, orthogonality를 비교한다.

### 지표

- A−QR의 상대 Frobenius norm과 Q의 직교성 오차
- 반사 전후 벡터 norm 및 제거할 성분의 크기
...
**복소 phase 처리와 reflector를 명시적으로 곱해 생기는 비용**
...
- 이전: [Lecture 09 — MATLAB]({% link _posts/trefethen/2026-09-15-trefethen-09-matlab.md %})
- 다음: [Lecture 11 — Least Squares Problems]({% link _posts/trefethen/2026-09-15-trefethen-11-least-squares-problems.md %})
