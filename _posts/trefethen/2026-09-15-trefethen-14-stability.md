...
forward error와 backward error를 분리해 측정한다.
...
- 배경: 수학적 문제와 계산 결과의 구분

## 먼저 답할 질문

- backward error를 만들 때 어느 입력을 perturb하는가?
- 작은 backward error가 큰 forward error와 공존할 수 있는가?

## 핵심 내용 체크리스트

- [ ] forward·backward error와 stability의 정의
- [ ] conditioning과 stability를 연결하는 논리
...
**예정 대상:** `nla::linear_system_backward_error` in `stability.*`
...
**확인할 비교:** 같은 근사해에 두 오차 지표를 적용하고 conditioning과 함께 해석한다.

### 지표

- 선형계의 normwise backward error
- 같은 실험의 forward error와 조건수를 함께 기록
...
**작은 residual만으로 작은 forward error를 단정**
...
- 이전: [Lecture 13 — Floating Point Arithmetic]({% link _posts/trefethen/2026-09-15-trefethen-13-floating-point-arithmetic.md %})
- 다음: [Lecture 15 — More on Stability]({% link _posts/trefethen/2026-09-15-trefethen-15-more-on-stability.md %})
