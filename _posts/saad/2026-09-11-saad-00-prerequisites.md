---
layout: post
title: "Saad 00 — Prerequisites"
permalink: /study/saad/00-prerequisites/
math: true
toc: true
status: outline
date: 2026-09-11 00:00:00 +0900
categories: [Solvers]
tags: [numerical-linear-algebra, iterative-methods, saad]
note_type: numerical-method
series: saad
series_order: 1
---

Source scope: Saad, 2nd ed. (2003), Ch. 1

## Prerequisite

- [Scalars, Vectors, Matrices and Maps]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-00-scalars-vectors-matrices-and-maps.md %})
- [Rank and Fundamental Subspaces]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-03-rank-and-fundamental-subspaces.md %})
- [Inner Product Geometry]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-05-inner-product-geometry.md %})
- [Orthogonal Decomposition]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-06-orthogonal-decomposition.md %})
- [Eigenvalues and Eigenvectors]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-07-eigenvalues-and-eigenvectors.md %})
- [Special Matrix Classes]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-09-special-matrix-classes.md %})
- [Spectral Theorem]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-10-spectral-theorem.md %})
- [Matrix and Operator Norms]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-12-matrix-and-operator-norms.md %})
- [Conditioning]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-13-conditioning.md %})

Dependency: 00 → [01]({% link _posts/saad/2026-09-11-saad-01-sparse-linear-systems.md %})

## Readiness checklist

- [ ] [Linear systems]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-03-rank-and-fundamental-subspaces.md %})
- [ ] [Matrix / vector norms; induced operator norm]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-12-matrix-and-operator-norms.md %})
- [ ] [Eigenvalues / eigenvectors; spectral radius]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-07-eigenvalues-and-eigenvectors.md %})
- [ ] [SPD matrices]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-10-spectral-theorem.md %})
- [ ] [Orthogonality]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-05-inner-product-geometry.md %})
- [ ] [Projections]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-06-orthogonal-decomposition.md %})
- [ ] [Condition number]({% link _posts/linear-algebra-review/2026-09-11-linear-algebra-13-conditioning.md %})

## Connection to next topic

- [Sparse Linear Systems]({% link _posts/saad/2026-09-11-saad-01-sparse-linear-systems.md %})

## Questions

## Final reconstruction
